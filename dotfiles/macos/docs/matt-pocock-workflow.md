...
**State of the world (2026-07-16):** the main flow runs **`/grill-with-docs → /to-spec → /to-tickets → /implement → /code-review`** — the flow Pocock demos and his `ask-matt` router encodes. **`/wayfinder`** is the on-ramp for a foggy effort **too big for one session**: in the [v1.1 launch video](https://www.youtube.com/watch?v=A8mokin_YOs) (2026-07-08) he calls it his front door ("default to Wayfinder instead" of grill-with-docs; "get obsessed with Wayfinder," especially for front-end work), but the written SKILL.md and `ask-matt` router keep it as the big-effort on-ramp, not the spine — so this doc follows the **written** position (a walk-back from the 2026-07-10 "wayfinder crowned default"; see Decision #10). Repo state (HEAD [`e9fcdf9`](https://github.com/mattpocock/skills/commit/e9fcdf9), 2026-07-14, still HEAD 2026-07-16): **40 skills** — 22 promoted (`engineering/` + `productivity/`, exactly the Claude Code plugin set) and 18 experimental. **Version needs mapping:** the **Claude Code plugin reports v1.2.0** (PR [#536](https://github.com/mattpocock/skills/pull/536) shipped the native plugin, on `main` since 2026-07-13) while the newest git tag and CHANGELOG entry remain **v1.1.0** — plugin 1.2.0 is today's `main`, not a newer release. v1.1.0 (2026-07-08) graduated `wayfinder`, `code-review`, `research`, and `prototype` out of `in-progress/` into the promoted set, renamed `to-prd`→`to-spec`, merged `to-plan`+`to-issues`→`to-tickets` (deleting `to-issues`), and sharpened `grilling` (a confirmation gate + a facts-vs-decisions split). v1.0.0 (2026-06-17) shipped the earlier breaking renames and the shared model-invoked layer (`grilling`, `domain-modeling`, `codebase-design`). Announced via the launch tweet ("mattpocock/skills v1.1 is out!"), the v1.1 launch video, and the aihero.dev v1.1 changelog post.
...
When to use: **the default entry point for a change in a repo** — a relentless one-question-at-a-time interview that sharpens a fuzzy idea and maintains `CONTEXT.md` + ADRs as decisions land, then goes straight to `/to-spec`. `/grill-me` instead only for plans outside a codebase ("I stopped using /grill-me for coding").
...
- Default entry: `/grill-with-docs` (relentless interview; maintains `CONTEXT.md` + ADRs as decisions land) goes straight to `/to-spec` — no map needed. `/grill-me` for plans outside a codebase.
...
**Or install as a Claude Code plugin** (on `main` since 2026-07-13, PR [#536](https://github.com/mattpocock/skills/pull/536)) — a **read-only, always-current** bundle of the 22 promoted skills only, reporting **v1.2.0**; `/setup-matt-pocock-skills` still runs once per repo:
...
| `to-questionnaire` | in-progress | Turn an unanswerable decision into a Markdown questionnaire for someone else, async or in a meeting; added 2026-07-14 |
...
---
Last updated: 2026-07-16 — reverted to grill-with-docs-led (walked back the 2026-07-10 Wayfinder-first framing; see Decision #10) and mapped the Claude Code plugin **v1.2.0** (native plugin on `main` since 2026-07-13, PR #536; newest git tag/CHANGELOG entry still v1.1.0). Synced from the `agent-hero` reference repo against mattpocock/skills HEAD `e9fcdf9` (2026-07-14). Sources: [github.com/mattpocock/skills](https://github.com/mattpocock/skills) (README + CHANGELOG + ask-matt/wayfinder SKILL.md + .claude-plugin/plugin.json, live 2026-07-16) · PRs #534/#535/#536/#538 · v1.1 launch video + launch tweet · [aihero.dev](https://www.aihero.dev) posts + talks/interviews of 2026-01→06 as linked above
