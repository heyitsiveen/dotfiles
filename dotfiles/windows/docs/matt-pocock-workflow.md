...
When to use: every coding task in a repo — relentless interview that maintains `CONTEXT.md` + ADRs as decisions land. `/grill-me` instead only for plans outside a codebase ("I stopped using /grill-me for coding"). Both signpost **up to `/wayfinder`** when the effort is too big to hold in one session.

**Step 3.** `/to-spec` (was `/to-prd`, renamed in v1.1.0) — model: same as step 2 · session: **same as step 2** ("Do not clear the context… just to write a spec")
When to use: multi-session work only — synthesizes the grilled conversation into a **spec** (opens "you may know this document as a PRD") published to the issue tracker. If the work fits one session, skip steps 3–4 and run `/implement` in place.
...
**On-ramps (before the chain)** — `/wayfinder` (new in v1.1.0) when the work is a **greenfield project or too big for one session**: it charts investigation tickets on the tracker and resolves them one at a time until the destination is clear, then points you at `/to-spec`. Positioned as a **situational on-ramp, not the main spine** — the grill-led idea→ship chain stays the front door. · `/triage` for issues and external PRs you didn't create · `/diagnosing-bugs` when something's broken.
...
- Greenfield projects or huge features too big for one session — start with `/wayfinder`.
...
### The main skill chain (current, 2026-07-08 / v1.1.0)
...
- Big work first: `/wayfinder` charts a greenfield/too-big effort into investigation tickets, then hands to `/to-spec`. It's an on-ramp, not part of the linear chain.
...
**`/grill-with-docs`** — the coding entry point. One-line body: run `/grilling` with `/domain-modeling` — so the interview simultaneously maintains `CONTEXT.md` and ADRs as decisions land. Signposts **up to `/wayfinder`** for an effort too big to hold in one session. "Might be the single coolest technique in this repo" (README).
...
**`/wayfinder`** — **graduated from `in-progress/` in v1.1.0** (renamed from `decision-mapping`; "decision map" was inaccurate — only one ticket type is a decision). Plans a chunk of work **too big for one agent session** by charting a route through a foggy problem — leading words **fog of war**, **frontier**, **the map**:
  ...
  - **No-fog early exit.** If the opening breadth-first grilling surfaces no fog, the journey fits one session — it stops and asks how you'd like to proceed rather than building a map nobody needs. Ends by recommending `/to-spec`. Positioned as a **situational on-ramp, not the main spine**.
...
- **Wayfinding — fog of war / frontier / the map / destination** — charting a route through a foggy problem: the map indexes investigation tickets, the frontier is what's takeable now, the destination fixes scope; plan, don't do.
...
Primary — repo (fetched 2026-07-09): [mattpocock/skills](https://github.com/mattpocock/skills) — **CHANGELOG.md v1.1.0** and README (2026-07-08), engineering/productivity/in-progress/misc/personal skill directories, releases `v1.0.0`/`v1.0.1` (2026-06-17) and **`v1.1.0` (2026-07-08)** · [mattpocock/sandcastle](https://github.com/mattpocock/sandcastle). v1.1 also announced via the launch tweet ("mattpocock/skills v1.1 is out!", user-supplied) and the aihero.dev v1.1 changelog post (page unreachable to automated fetch, 404/402 — not quoted).
...
---
Last updated: 2026-07-09 — sources: [github.com/mattpocock/skills](https://github.com/mattpocock/skills) (CHANGELOG v1.1.0 + README, 2026-07-08) · v1.1 launch tweet · [aihero.dev](https://www.aihero.dev) posts + talks/interviews of 2026-01→06 as linked above
